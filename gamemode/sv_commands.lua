...
cider.command.add("note", "b", 1, function(ply, ...)
	...
	timer.Create("Note Cleanup: " .. index, GM.Config["Note Fade Speed"], 0, noteclean, entity, index);
...
cider.command.add("door", "b", 1, function(ply, arguments)
	...
	if not( ValidEntity(door) and cider.entity.isDoor(door) and ply:GetPos():Distance( ply:GetEyeTraceNoCursor().HitPos ) <= 128	) then
...
cider.command.add("entity", "b", 2, function(ply, arguments)
	...
	local entity = ply:GetEyeTraceNoCursor().Entity
	if not (ValidEntity(entity) and cider.entity.isOwnable(entity) and ply:GetPos():Distance( ply:GetEyeTraceNoCursor().HitPos ) <= 128) then
	...
	datastream.StreamToClients(ply,"cider_Access_Update",detailstable)
	return a,b
